...
[asy] import graph; size(6cm); draw((0,0)--(0,10),EndArrow); draw((0,0)--(0,-10),EndArrow); draw((0,0)--(10,0),EndArrow); draw((0,0)--(-10,0),EndArrow); Label f; f.p=fontsize(6); xaxis(-10,10); yaxis(-10,10); real f(real x) { return x^2-5; } draw(graph(f,-4,4),blue+linewidth(1)); draw(circle((0,0),5),red); dot(scale(.7)*"$a$",(0,5),NE); dot(scale(.7)*"$-a$",(0,-5),N); dot(scale(.7)*"$a$",(5,0),NE); dot(scale(.7)*"$-a$",(-5,0),SE); [/asy]
...
From observing the graph of the two functions, we notice that there will be exactly 3 solutions when the curvature of $y = x^2-a$ is greater than the curvature of the circle $x^2+y^2 = a^2$ near $x=0$ (this means the graph of $y=x^2-a$ will reside within the circle). Curvature can be calculated by:
