...
2 Solutions
2.1 Solution 1
2.2 Solution 2(ADD DIAGRAM)
2.3 Remark
3 Video Solution by icematrix
4 See also
Problem
...
next we can find the volume of the frustum and of the sphere and we know $V_{\text{frustum}}=2V_{\text{sphere}}$ so we can solve for $s$ using $V_{\text{frustum}}=\frac{\pi*h}{3}(R^2+r^2+Rr)$ we get:
...
$r=\dfrac{3\pm\sqrt{(-3)^2-4*1*1}}{2*1}$ so
