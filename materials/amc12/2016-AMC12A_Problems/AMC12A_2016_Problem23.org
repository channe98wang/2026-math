...
7.1 Solution 5.1 (better explanation)
8 Solution 7: More WLOG, Complementary Probability
9 Solution 8: 3D geometry
10 Solution 9(Fastest solution if you have no time): Stick Solution
11 Video Solution by Punxsutawney Phil
12 Video Solution by The Art of Problem Solving
13 See Also
Problem
...
[asy] pair A = (0,0); pair B = (1,0); pair C = (1,1); pair D = (0,1); pair E = (0,0); draw(A--B--C--D--cycle); draw(B--D,dashed); fill(B--D--C--cycle,gray); label("$0$",A,SW); label("$a$",B,S); label("$a$",D,W); label("$y$",(0,.5),W); label("$x$",(.5,0),S); label("$x+y>a$",(5/7,5/7)); [/asy]
