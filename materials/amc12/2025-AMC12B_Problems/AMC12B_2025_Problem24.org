...
2 Note
3 Solution 1
4 Solution 2 (AoPS Precalculus)
5 Solution 3 (quick but risky)
6 Solution 4 (Unit-period parametrization)
7 Solution 5 (Risky)
...
Note
This problem's solution logic is basically the same as 1991 AIME Q4 .
Solution 1
...
Each period of the sin function is $\frac{1}{10}$ , so 200 periods, 2 intersections each (just draw it out) - 400. But there is 1 overlap in two periods at $(1,0)$ , so subtract 1 -> 399 (can check end behavior if time)
~ScoutViolet
