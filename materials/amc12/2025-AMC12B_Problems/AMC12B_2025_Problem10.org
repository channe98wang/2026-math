...
1 Problem
2 Diagram
3 Solution 1 (Mass Points)
4 Solution 2 (Similar Triangles)
5 Solution 3 (Similar Triangles)
6 Solution 4 (Similar Triangles)
7 Solution 5 (Similar Triangles)
8 Solution 6 (Coordinates)
9 Solution 7 (Symmedian and Bary)
10 Solution 8 (Trigonometry Bash)
11 Solution 9 (Easier to Understand, Using Similar 30-60-90 Triangles)
12 Solution 10 (Ruler)
13 Solution 11 (Law of Cosines Bash)
14 Solution 12 (Vectors and Projections)
15 Solution 13 (Straightforward Coordinate Geometry)
16 Video Solution 1 by SpreadTheMathLove
17 Video Solution 2 (Fast and Easy)
18 See Also
Problem
...
[asy] /* Made by MRENTHUSIASM */ size(250); pair A, B, C, D, E, F; A = (0,1); B = origin; C = (sqrt(3),0); D = foot(B,A,C); E = midpoint(A--B); F = intersectionpoint(B--D,C--E); markscalefactor=0.008; draw(rightanglemark(A,B,C)^^rightanglemark(B,D,C),red+linewidth(1.25)); draw(A--B--C--cycle,linewidth(1.25)); draw(B--D^^C--E,linewidth(1.25)); label("$1$",midpoint(A--E),1.5*W,red); label("$1$",midpoint(E--B),1.5*W,red); label("$2\sqrt{3}$",midpoint(B--C),1.5*S,red); label("$x$",midpoint(D--F),1.25*dir(-45),red); label("$y$",midpoint(F--B),1.5*dir(-10),red); label("$30^{\circ}$",C,7*dir(165),red); label("$60^{\circ}$",A,4*dir(-63),red); label("$1$",midpoint(A--D),1.5*midpoint(A--D),red); label("$3$",midpoint(D--C),1.5*dir(70),red); label("$A$",A,1.5*NW); label("$B$",B,1.5*SW); label("$C$",C,1.5*SE); label("$D$",D,1.5*NE); label("$E$",E,1.5*W); label("$F$",F,2*dir(-75)); [/asy]

 From here, we will proceed with mass points . Throughout this solution, we will use $W_P$ to denote the weight of point $P.$
...
From the area formula we get $1/2 \cdot 6\cdot6\sqrt{3} =1/2\cdot12\cdot h$
$h = 3\sqrt{3}$
The shortest side is $AB$ with side length $6$ , so the median from $C$ to $AB$ will be at $(3,0)$ .
Finding the equation of the line through that point to C, find that the slope is $\frac{6\sqrt{3}-0}{6-3}=2\sqrt3$ . 
Plugging the point $(3,0)$ into $y=2\sqrt{3}x+b$ , we get that $0=6\sqrt{3}+b$ , which simplifies to $b=-6\sqrt{3}$ So, the equation of the line of the median from $C$ is $y=2\sqrt{3}x-6\sqrt3$
...
\[\sqrt{x^2 - 2\sqrt{3}x + 12} + \sqrt{x^2 - \sqrt{3}x + 1} = \sqrt{13}\]

Algebra: $x = \frac{4\sqrt{3}}{7}$ $y = \frac{3\sqrt{3}}{7}$ $x$ is defined to be minimum of those lengths, so $x = \frac{3\sqrt{3}}{7}$ .
Hence $\frac{x}{x+y} = \boxed{\textbf{(A) } \dfrac{3}{7}}$ .
~clod
...
Let the triangle have vertices A, B, and C on the 60,90,and 30 angles, respectively. Let E be the intersection of the median from C and AB, let F be the intersection of the altitude from B to AC, and let P be the intersection of CE and BF. WLOG let A be (0,2), B be (0,0), and C be (2√3,0). In order to find the ratio of PF to BF, we need to find the coordinates of F and P. First, slope of AC is -1/√3, so the slope of BF is √3. So, the equation for AC is y = (-1√3)x+2 and for BF y = √3x. Finally, E is the midpoint of AB, E is at (0,1), the slope of CE is -1/2√3, and the equation for CE is y = (-1/2√3)x+1. Setting the equations for AC and BF equal, we find the intersection is F at (√3 /2,3/2). Setting the equations for CE and BF equal, we find the intersection is P at (2√3 /7, 6/7). Because BF goes through the origin, we can find BP/BF by dividing the y-coordinates of P and F, getting 4/7. Thus, PF/BF = 1-BP/BF = 1-4/7 = 3/7.
~spiritmoon_OSU
