...
1 Problem
2 Diagram
3 Solution 1 (Right Triangles)
4 Solution 2 (Law of Cosines)
5 Solution 3 (Double Angle Identities)
6 Solution 4 (Vector Methods)
7 Video Solution 1 (Quick and Simple)
8 Video Solution 1 (Smart and Simple)
9 See Also
Problem
...
\[\cos(\angle CMD)=\frac{MO}{MC}=\boxed{\textbf{(B) } \frac13}.\]
...
\[\cos(\angle CMD) = \frac{CM^2 + DM^2 - CD^2}{2(CM)(DM)} = \boxed{\textbf{(B) } \frac13}.\]
...
\[\cos(\angle CMD) = \frac{2}{3} - \frac13 = \boxed{\textbf{(B) } \frac13}.\]

~Misclicked
Solution 4 (Vector Methods)
Without loss of generality, let tetrahedron $ABCD$ lie in three-dimensional space with vertices $A(-1, 1, 1)$ , $B(1, 1, -1)$ , $C(1, -1, 1)$ , and $D(-1, -1, -1)$ . 
...
\[\cos(\angle CMD) = \frac{2}{6} = \boxed{\textbf{(B) } \frac13}.\]
