...
[asy] /* Made by MRENTHUSIASM */ size(350); for (int y = 20; y >= 1; --y) { for (int x = 0; x <= 20-y; ++x) { dot((x,y),green+linewidth(4)); } } for (int y = 0; y >= -20; --y) { for (int x = 1; x <= y+20; ++x) { dot((x,y),blue+linewidth(4)); } } for (int y = 20; y >= 0; --y) { for (int x = y-20; x <= -1; ++x) { dot((x,y),purple+linewidth(4)); } } for (int y = -1; y >= -20; --y) { for (int x = -y-20; x <= 0; ++x) { dot((x,y),red+linewidth(4)); } } dot(origin,black+linewidth(4)); [/asy]
