...
Alternate Solution
Let the coordinates of $A$ and $B$ be $(x_A, y_A)$ and $(x_B, y_B)$ , respectively. Since the median of the points lies on the origin, $x_A + x_B = y_A + y_B = 0$ and expanding $y_A + y_B$ , we find:
...
we find the distance to be $\sqrt{50(x_A - x_B)^2}$ . Expanding this yields $5\sqrt{2(x_A^2 + x_B^2 - 2x_A x_B)} = \boxed{5\sqrt{2}}$ .
See Also
