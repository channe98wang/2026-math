...
Contents
1 Problem
2 Solution (not a proof, but good enough for AMCs)
3 Solution
4 Alternative Solution
5 Clarification
6 See also
Problem
The tower function of twos is defined recursively as follows: $T(1) = 2$ and $T(n + 1) = 2^{T(n)}$ for $n\ge1$ . Let $A = (T(2009))^{T(2009)}$ and $B = (T(2009))^A$ . What is the largest integer $k$ for which
...
Since $\varepsilon$ is still a positive number, we can take the $\log$ of it one last time before it becomes negative. So, after a long and very tiring journey (for both you, the reader, and me, the writer) we have $k$ (the number of times we took the $\log$ ) $= 2013 \Rightarrow \boxed{\text{E}}$ .
~TheAsian
See also
