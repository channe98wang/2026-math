...
12 Video Solution by Power Solve (easy to digest!)
13 Video Solution (🐉⚡Under 5 min⚡🐉)
...
Solving, we get $x=\frac{\sqrt{3}-1}{2}$ and $y=\frac{\sqrt{3}+1}{2}$ . Thus, we find $\frac{x}{y}=\frac{\sqrt{3}-1}{\sqrt{3}+1}=\frac{(\sqrt{3}-1)^2}{(\sqrt{3}+1)\cdot(\sqrt{3}-1)}=\frac{4-2\sqrt{3}}{2}=\boxed{\textbf{(C) }2-\sqrt3}$ .
~vadava_lx
Solution 5
