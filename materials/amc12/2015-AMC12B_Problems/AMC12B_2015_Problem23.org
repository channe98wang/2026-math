...
2.1 Supplement
3 Solution 2
4 Simplification of Solution 2
5 Video Solution
6 See Also
Problem
...
\[abc = 2(ab+bc+ac) \quad \text{ or } \quad (a-2)bc = 2a(b+c).\]
...
Notice $\emph{\text{immediately}}$ that $b, c > q$ . This is our key step.
Then we can say $b=q+d$ , $c=q+e$ . If we clear the fraction about b and c (do the math), our immediate result is that $de = q^2$ . Realize also that $d \leq e$ .
