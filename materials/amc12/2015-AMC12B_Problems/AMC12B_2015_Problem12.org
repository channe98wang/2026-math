...
3 Solution 1
4 Solution 2
5 Solution 3 (Vieta's Formula)
6 Solution 3 (Vieta's Formula)
7 See Also
Problem
...
\[\dfrac{-b}{2a}+\dfrac{\sqrt{b^2-4ac}}{2a}, \dfrac{-b}{2a}-\dfrac{\sqrt{b^2-4ac}}{2a}.\]
...
Now, to maximize this, it is clear that $b=9.$ Also, we must have $a=8, c=7$ (or vice versa). The reason $a,c$ have to equal these values instead of larger values is because each of $a,b,c$ is distinct.
