...
2 Solution 1
2.1 Note
3 Solution 2 (using the answer choices)
4 Solution 3 (extremely simple)
...
Note
You can more easily count the number of triples $(A, B, C)$ by noticing that there are $9 - B$ possible values for $A$ and $10 - B$ possible values for $C$ once $B$ is chosen. Summing over all $B$ , the number is

\[9\cdot 10 + 8\cdot 9 + \ldots + 1\cdot 2 = 2\left(\binom{2}{2} + \binom{3}{2} + \ldots + \binom{10}{2}\right).\]
...
\[(9 \cdot 10) + (8 \cdot 9) + (7 \cdot 8) + (6 \cdot 7) + (5 \cdot 6) + (4 \cdot 5) + (3 \cdot 4) + (2 \cdot 3) + (1 \cdot 2)\]
