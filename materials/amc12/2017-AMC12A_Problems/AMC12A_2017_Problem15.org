...
4 Solution 3(More Detailed Answer of Solution 1)
5 See Also
Problem
...
[asy] Label f; f.p=fontsize(6); xaxis(-5,5,Ticks(f, 1.0)); yaxis(-8,8,Ticks(f, 2.0)); real f(real x) { return sin(x); } draw(graph(f, -5,5)); real g(real x) { return 2*cos(x); } draw(graph(g, -5,5)); real h(real x) { return 3*tan(x); } draw(graph(h, -1.2,1.2)); draw(graph(h, 1.94, 4.34)); draw(graph(h, -4.34, -1.94)); [/asy]
