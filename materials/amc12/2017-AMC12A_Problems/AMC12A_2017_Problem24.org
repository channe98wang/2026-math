...
Let $Z$ be the intersection of $AC$ and $BD$ . First, from $ABCD$ being a cyclic quadrilateral, we have that $\triangle BCZ \sim \triangle AZD$ , $\triangle BZA \sim \triangle CDZ$ . Therefore, $\frac{2}{BZ} = \frac{8}{AZ}$ , $\frac{6}{CZ} = \frac{3}{BZ}$ , and $\frac{2}{CZ} = \frac{8}{DZ}$ , so we have $BZ = \frac{1}{2}CZ$ , $AZ = 2CZ$ , and $DZ = 4CZ$ . By Ptolemy's Theorem,

\[(AB)(CD) + (BC)(DA) = (AC)(BD) = (AZ + ZC)(BZ + ZD)\]

\[\rightarrow 3 \cdot 6 + 2 \cdot 8 = 34 = \left(2CZ + ZC\right)\left(\frac{1}{2}CZ + 4CZ\right) = \frac{27}{2}CZ^2.\]

Thus, $CZ^2 = \frac{68}{27}$ . Then, by Power of a Point, $GX \cdot XC = BX \cdot XD = \frac{3}{4} \cdot \frac{1}{4}BD^2 = \frac{3}{16} \cdot \left(\frac{9}{2}CZ\right)^2 = \frac{9 \cdot 17}{16}$ . So, $XG = \frac{9 \cdot 17}{16XC}$ .
