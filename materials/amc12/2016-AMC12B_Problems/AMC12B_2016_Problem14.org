...
4 Solution 2
5 Solution 3
6 Solution 4 (no AM-GM or Calculus)
7 Solution 5 (Quadratic formula)
8 Video Solution by CanadaMath (Problem 11-20)
9 See Also
Problem
...
Let $r$ be the common ratio. If the second term is $1$ , the first must be $\frac{1}{r}$ . By the infinite geometric series formula, the sum must be

\[S=\frac{\frac{1}{r}}{1-r}\]

This equals $\frac{1}{r(1-r)}$ . To find the minimum value of S, we must find the maximum value of the denominator, $r(1-r)$ , which is $\frac{1}{4}$ , completing the square. Thus, the minimum value of $S$ is $\boxed{\textbf{(E)}\ 4}$ .
