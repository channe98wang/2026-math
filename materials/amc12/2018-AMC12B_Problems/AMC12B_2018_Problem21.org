...
1 Problem
2 Diagram
3 Solution 1 (Coordinate Geometry)
4 Solution 2
5 Solution 3
6 See Also
Problem
...
\[\sqrt{(a-6)^2+\left(a-\frac52\right)^2}=\frac{13}{2}-a.\]

Solving this equation, we have $a=4,$ from which $M=(4,4).$
Finally, we apply the Shoelace Theorem to $\triangle MOI:$

\[[MOI]=\frac12\left|\left(4\cdot\frac52+6\cdot2+2\cdot4\right)-\left(4\cdot6+\frac52\cdot2+2\cdot4\right)\right|=\boxed{\textbf{(E)}\ \frac72}.\]
...
\[[MOI]=\frac12\cdot MI\cdot h_O=\frac72.\]
