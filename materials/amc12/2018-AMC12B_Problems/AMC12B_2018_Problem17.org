...
From $\frac ab < \frac pq,$ we have $bp-aq>0,$ or

\[bp-aq\geq1. \hspace{15mm} (1)\]

From $\frac pq < \frac cd,$ we have $cq-dp>0,$ or

\[cq-dp\geq1. \hspace{15mm} (2)\]

Since $bc-ad=1,$ note that:
- Multiplying $(1)$ by $d,$ multiplying $(2)$ by $b,$ and adding the results, we get $q\geq b+d.$
- Multiplying $(1)$ by $c,$ multiplying $(2)$ by $a,$ and adding the results, we get $p\geq a+c.$
To minimize $q,$ we set $q=b+d,$ from which $p=a+c.$ Together, we can prove that

\[\frac{a+c-1}{b+d}\leq\frac ab<\frac{a+c}{b+d}<\frac cd\leq\frac{a+c+1}{b+d}. \hspace{15mm} (\bigstar)\]
...
- $\frac{a+c-1}{b+d}\leq\frac ab$ and $\frac cd\leq\frac{a+c+1}{b+d}$
...
\[\frac{8}{16}<\frac59<\frac{9}{16}<\frac47<\frac{10}{16},\]

from which $p=9.$ Therefore, the answer is $q-p=\boxed{\textbf{(A) } 7}.$
Alternatively, refer to the section in Solution 1 for further generalizations.
~pieater314159 ~MRENTHUSIASM
Solution 3 (Substitution)
Inverting the given inequality we get

\[\frac{7}{4} < \frac{q}{p} < \frac{9}{5},\]
...
Clearly $p>k.$ We will now substitute $p = k + x$ to get
...
\[\frac{1}{18} < \frac{p}{q}-\frac{1}{2} < \frac{1}{14}.\]

Then for $q$ to be minimized, $\frac{p}{q}-\frac{1}{2}$ must be $\frac{1}{16},$ so $\frac{p}{q}$ is $\frac{9}{16}$ and $q-p$ is $\boxed{\textbf{(A) } 7}.$
...
\[\begin{cases} \frac{a}{b} > 1 &\Rightarrow \frac{a}{b} > \frac{a + 1}{b + 1} > \frac{a + 2}{b + 2} > \cdots \\ \frac{a}{b} < 1 &\Rightarrow \frac{a}{b} < \frac{a + 1}{b + 1} < \frac{a + 2}{b + 2} < \cdots \end{cases}\]
