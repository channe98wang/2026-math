...
which would give us that $x=\frac{1}{2}.$ Adding up $CF$ and $EF,$ we get $2+\frac{1}{2}=\boxed{\mathrm{(D)}\ \frac{5}{2}}$ again. Note that we know $DE=2-x$ because $\triangle EFG\cong \triangle EAG$ by $\text{HL.}$
...
\[EGC= \frac{EG\cdot{GC}}{2} = \frac{\sqrt{5x^2 + 5}}{2}\]
