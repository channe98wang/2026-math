...
Contents
1 Problem
2 Solution 1: Cross-Multiplication
...
\[\frac{1}{x}=\frac{x}{\sqrt{x^2+1}}\implies x^4-x^2-1=0.\]

Let $y=x^2$ and note that $y^2-y-1=0$ has solutions $y=\frac{1\pm\sqrt{5}}{2}$ . Since we want the positive answer, then $x^2=y=\frac{1+\sqrt{5}}{2}$ and $\frac{1}{x^2}=\boxed{\textbf{(C)} \frac{\sqrt{5}-1}{2}}$ .
