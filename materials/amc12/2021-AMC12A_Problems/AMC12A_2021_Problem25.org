...
3 Solution 2 (Solution 1 but Fewer Notations)
4 Solution 3 (Fast)
5 Solution 4 (Guesswork)
6 Solution 5 (Simple Explanation)
7 Calculus Remark
8 Video Solutions
9 See also
Problem
...
Now, we rewrite $f(n)$ as

\[f(n)=\frac{d(n)}{\sqrt [3]n}=\frac{\prod_{i=1}^{k}(e_i+1)}{\prod_{i=1}^{k}p_i^{e_i/3}}=\prod_{i=1}^{k}\frac{e_i+1}{p_i^{{e_i}/3}}.\]

As $f(n)>0$ for all positive integers $n,$ note that $f(a)>f(b)$ if and only if $f(a)^3>f(b)^3$ for all positive integers $a$ and $b.$ So, $f(n)$ is maximized if and only if
