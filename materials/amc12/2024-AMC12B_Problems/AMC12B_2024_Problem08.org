...
1 Problem
2 Solution 1
3 Solution 2 (Change of Base)
4 Solution 3 (Using Variables)
5 Video Solution 1 by TheBeautyofMath
6 See also
Problem
...
\[\frac{a \cdot a \log_3 2}{a+a \log_3 2}=2.\]

Notice that we can factor out an $a$ in the numerator and denominator, if $a \neq 0,$ and doing so yields

\[\frac{a \log_3 2}{1+\log_3 2}=2.\]

We know that $1= \log_3 3,$ so putting that in gives us

\[\frac{a \log_3 2}{\log_3 3+\log_3 2}=2 \implies \frac{a \log_3 2}{\log_3 6}=2.\]

So, $a=2 \cdot \frac{\log_3 6}{\log_3 2}$ , which, using the change of base formula, is equivalent to $2 \cdot \log_2 6,$ thus,

\[a=2 \cdot \log_2 6= \log _2 6^2= \log _2 36.\]

Finally, using our original definition of $a,$ we have

\[a = \log_2 x=\log_2 36,\]

so $x=\boxed{\textbf{(C) }36}.$
~hdanger
