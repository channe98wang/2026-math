...
\[\sin(2x)=\sin(x)+\sin(3x)=2\sin(2x)\cos(x)\]

So either $\sin(2x)=0$ , i.e. $x\in\{0,\pi\}$ or $\cos(x)=\tfrac 12$ , i.e. $x\in \{\pi/3, 5\pi/3\}$ .
...
Solution 4
Let $a=\cos(x)+i\sin(x)$ , so by De Moivre $P(x)=a^3-a^2+a+1$ . The problem essentially asks for the number of real roots of $P$ which lie on the complex unit circle.
