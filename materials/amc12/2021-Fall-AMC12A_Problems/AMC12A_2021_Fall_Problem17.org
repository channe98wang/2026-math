...
\[b^4\leq 16c^2\leq 64b.\]

Since $b^4\leq 64b,$ it follows that $b\leq4.$ We apply casework to the value of $b:$
...
[asy] unitsize(2); Label f; f.p=fontsize(6); xaxis("$x$",0,5,Ticks(f, 1.0)); yaxis("$y$",0,5,Ticks(f, 1.0)); real f(real x) { return 0.25x^2; } real g(real x) { return 2*sqrt(x); } dot((1,1)); dot((2,1)); dot((1,2)); dot((2,2)); dot((3,3)); dot((4,4)); draw(graph(f,0,sqrt(20))); draw(graph(g,0,5)); [/asy]
...
\[\left\{ \begin{array}{ll} b^2 - 4 c \leq 0 \\ c^2 - 4 b \leq 0 \end{array} \right..\]
