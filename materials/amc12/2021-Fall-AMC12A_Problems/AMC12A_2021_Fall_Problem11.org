...
Then, use the Pythagorean Theorem . In $\triangle OMA$ , we have 
...
\[19^2 - \frac{AD^2}{4} = 17^2 - \frac{AD^2}{16}\]

and $AD=\boxed{\textbf{(E) }8 \sqrt{6}}$ .
...
in which the $34x$ and $-x^2$ terms magically cancel when we subtract the first equation from the second equation. Thus, now we have
...
Substituting $y^{2}$ into $x^{2}+y^{2}=19^{2}$ , we obtain $x=\sqrt{96}=4\sqrt{6}$ . However, this is only half of the chord length, so we must double it to obtain $\boxed{\textbf{(E)} \: 8\sqrt{6}}.$
-helpmebro
