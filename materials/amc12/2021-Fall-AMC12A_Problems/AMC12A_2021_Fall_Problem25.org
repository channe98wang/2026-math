...
Contents
1 Problem
2 Solution 1 (Complete Residue System)
3 Solution 2 (Symmetric Congruent Numbers and Interpolation)
4 Solution 3
5 Solution 4
6 Solution 5 (Principle of Inclusion-Exclusion)
7 Video Solution
8 See Also
Problem
...
\[\{k+4(0),k+4(1),k+4(2),\ldots,k+4(m-1)\}\]

is the complete residue system modulo $m$ for all integers $k.$
...
We conclude that $D(m)=\frac1m\cdot[m(m-1)(m-2)(m-3)]=(m-1)(m-2)(m-3),$ so

\[q(x)=(x-1)(x-2)(x-3)=c_3x^3+c_2x^2+c_1x+c_0.\]
...
Taking $\frac{(1.2)-(1.1)}{2}$ , $\frac{(1.3)-(1.2)}{2}$ , $\frac{(1.4)-(1.2)}{2}$ , we get 
...
Consider how many total ordered quadruples of distinct integers exist in the range $[1,m]$ :
- We choose 4 distinct numbers from $m$ possible values: $\binom{m}{4}$ - We arrange these 4 numbers in order: $4!$
