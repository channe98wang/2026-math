...
3 Solution 2
4 Solution 3
5 Solution 4 (Pattern Observation)
6 See Also
Problem
...
Solving for x, we get that $x = \frac{1}{3},$ and so $A_{1} = (2/3,0)$ .
...
\[1+12 \cdot \frac{2}{3} = 9.\]

Similarly, the discriminant of

\[(x \sqrt{3})^{2} = x+a_{3}\]

is $1+12 \cdot a_{3} = 25,$ and $1+12 \cdot a_{4} = 49$ .
...
for some integer $n-1$ . We will prove that this is true for $n$ as well.
