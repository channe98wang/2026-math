...
\[\tan\alpha=\frac{3}{4} \text{ and } \tan\beta=\frac{7}{24}.\]
...
~ luckuso
Added Note: It should be $cos(B) = \frac{24}{25} = 2 \cdot \frac{12}{25} = 2 \cdot \frac{3}{5} \cdot \frac{4}{5} = 2 \cdot sin(A) \cdot cos(A) = sin(2A) = cos(90^{\circ} - 2A)$ . $sin(B) is actually 7/25!
