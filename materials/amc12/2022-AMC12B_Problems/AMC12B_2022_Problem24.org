...
11 Solution 10 (cheese)
12 Supplement (Explanation of why cos(2π/7) + cos(4π/7) + cos(6π/7) = -1/2)
13 Video Solution
14 See Also
Problem
...
\[7 \cdot 2^4 \sin^4 \frac{\pi}{7} + 7 \cdot 2^4 \sin^4 \frac{2 \pi}{7} + 7 \cdot 2^4 \sin^4 \frac{3 \pi}{7} = 7 \cdot 2^4 \left(\sin^4 \frac{\pi}{7} + \sin^4 \frac{2 \pi}{7} + \sin^4 \frac{3 \pi}{7}\right) = 7 \cdot 2^4 \cdot \frac{21}{16} = \boxed{\textbf{(C) }147}.\]
...
\[\left(\cos\dfrac{2\pi n}{7}, \sin\dfrac{2\pi n}{7}\right)\]

for integer $n.$ Then, we notice there are three types of diagonals: the ones with chords of arcs $\dfrac{2\pi}{7}, \dfrac{4\pi}{7},$ and $\dfrac{6\pi}{7}.$ We notive there are here are $7$ of each type of diagonal. Then, we use the pythagorean theorem to find the distance from $\left(\cos\frac{2\pi n}{7}, \sin\frac{2\pi n}{7}\right)$ to $(1, 0)$ :
...
\[4\cos^2\frac{2\pi n}{7}-8\cos\frac{2\pi n}{7}+4=2\left(\cos\frac{4\pi n}{7}+1\right)-8\cos\frac{2\pi n}{7}+4=2\cos\frac{4\pi n}{7}-8\cos\frac{2\pi n}{7}+6.\]

Summing this up for $n=1,2,3,$
...
(These equalities are based on $\cos\theta=\cos(2\pi-\theta)$ and $\cos\frac{2\pi}{7}+\cos\frac{4\pi}{7}+\cos\frac{6\pi}{7}=-\frac{1}{2}.$ ) (For explanation of this see supplement .)
...
- Prof. Joker
Solution 10 (cheese)
There are $7$ segments with length $2\sin{\frac{\pi}{7}}$ , $7$ segments with length $2\sin{\frac{2\pi}{7}}$ , and $7$ segments with length $2\sin{\frac{3\pi}{7}}$ . The sum is $112(\sin^4\frac{\pi}{7}+\sin^4\frac{2\pi}{7}+\sin^4\frac{3\pi}{7})$ Note that $\frac{3}{7} = 0.425871...$ , so $\sin\frac{3\pi}{7} \approx \sin\frac{\pi}{2}$ , but a little less (The actual value is approximately $0.975$ ), and based on the answer choices, we can conclude that our anser will be a little greater than $1$ , so we make the educated guess of $\boxed{\textbf{(C) }147}$
