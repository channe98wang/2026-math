...
Contents
1 Problem
2 Solution 1 (Solves for Floor(x))
3 Solution 2 (Solves for x)
4 Solution 3 (Solves for x)
5 Remark
6 Video Solution 1 (Geometry)
7 Video Solution 2
8 Video Solution 3 (by Art of Problem Solving)
9 Video Solution 4
10 See Also
Problem
...
\[\sum_{w=1}^{W}w(1+k)=(1+k)\cdot\sum_{w=1}^{W}w=(1+k)\cdot\frac{W(W+1)}{2}=420. \hspace{10mm}(\bigstar)\]
...
Summing over $n\in \{1,2,\ldots , N\}$ we get

\[\tfrac 12 N(N+1) < 420 = \tfrac 12 N(N+1)\zeta < \tfrac 12 N(N+3)\]

From the right inequality we get $27< N$ and from the left one we get $N<29$ . Thus $N=28$ . Using this in the middle equality we get $\zeta = \tfrac{30}{29}$ . Since $\zeta$ satisfies $a\zeta^2-\zeta+1=0$ , we get
