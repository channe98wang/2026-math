...
2 Solution 1
3 Solution 2
4 Solution 3 (No Logarithm)
5 Video Solution by OmegaLearn (Logarithmic Tricks)
6 Video Solution by hippopotamus1
7 Video Solution by The Power of Logic
8 See Also
Problem
...
Let $x=t$ be the larger solution. Since exponential functions outgrow logarithmic functions, we have $f(x)<g(x)$ for all $x>t.$ By the Intermediate Value Theorem , we get $t\in(2,4),$ from which

\[S=\sqrt2+t\in\left(\sqrt2+2,\sqrt2+4\right).\]
