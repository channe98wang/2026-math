...
by the commutative property(this is the same as the equation given in the problem. We are just rearranging). So we can set $\frac{1}{x} = x$ which gives us $x = \pm 1$ which is answer option $\boxed{\mathrm{(E) \ } \{-1,1\}}.$
See also
