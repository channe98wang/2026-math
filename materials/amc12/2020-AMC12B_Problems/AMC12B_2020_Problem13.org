...
3 Solution 2 (Change of Base Formula)
4 Solution 3 (Observations)
5 Solution 4 (Solution 3 but More Detailed)
6 Video Solution (HOW TO THINK CREATIVELY!!!)
7 Video Solution by TheBeautyOfMath
8 Video Solution by Sohil Rathi
9 See Also
Problem
...
\[\sqrt{\log_2{6}+\log_3{6}}>\sqrt{2.5+1.5}=2.\]

Only choices $\textbf{(D)}$ and $\textbf{(E)}$ are greater than $2,$ but $\textbf{(E)}$ is certainly incorrect: If we compare the squares of the original expression and $\textbf{(E)},$ then they are clearly not equal. So, the answer is $\boxed{\textbf{(D) } \sqrt{\log_2{3}}+\sqrt{\log_3{2}}}.$
~Baolan
